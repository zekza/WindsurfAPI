...
import { isStickyEnabled, getStickyBinding, clearStickyBinding } from './account/sticky-session.js';
...
export function getApiKey(excludeKeys = [], modelKey = null, callerKey = null) {
  ...
  if (callerKey && isStickyEnabled()) {
    ...
    if (bound && !excludeKeys.includes(bound.apiKey)) {
      ...
      if (acct) {
        ...
        if (limit > 0 && used < limit && !isRateLimitedForModel(acct, modelKey, now)
          && (!modelKey || isModelAllowedForAccount(acct, modelKey))) {
    ...
    if (bound) clearStickyBinding(callerKey, modelKey);
  }
  const candidates = [];
